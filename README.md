...
| **Pipeline Project** | **LLM Data Pipeline (Person 1 Responsibility)**<br>ระบบ Pipeline รวบรวมข้อมูลหลากฟอร์แมต (PDF, Docx, HTML, TXT, MD) และทำ Data Cleaning กรองขยะตาม Pydantic Data Contract | `Pydantic v2`, `PyPDF`, `BeautifulSoup4`, `PyTest` | ![Completed](https://img.shields.io/badge/Status-Completed-success?style=flat-square) | [📂 `My-Pipeline.../`](./My-Pipelineproject-collection-cleaning/) |
| **Final Project** | **ATCS Capstone Final Project**<br>โครงงานโครงงานวิชาหัวข้อขั้นสูงทางซอฟต์แวร์คอมพิวเตอร์ | `End-to-End LLM/RAG/Agent Project` | ![Ongoing](https://img.shields.io/badge/Status-Ongoing-blue?style=flat-square) | [📂 `Final-Project/`](./Final-Project/) |
...
```mermaid
flowchart LR
    subgraph S1["1. Collection & Cleaning"]
        RAW["Multi-format Files (.pdf, .docx, .html, .txt)"] --> CLEAN["Text Cleaning & Normalization (PyThaiNLP)"]
    end
    
    subgraph S2["2. Indexing & Storage"]
        CLEAN --> CHUNK["Word-aware Chunking (10-15% Overlap)"]
        CHUNK --> EMB["Dense Vector (BAAI/bge-m3) + Sparse BM25"]
        EMB --> VDB["FAISS Index + Metadata Store"]
    end

    subgraph S3["3. Hybrid Retrieval & Rerank"]
        QUERY["User Question"] --> SEARCH["Hybrid Search (Dense + Sparse)"]
        VDB --> SEARCH
        SEARCH --> RERANK["Cross-Encoder Re-ranker (Top-K=3)"]
    end

    subgraph S4["4. Generation & Guardrails"]
        RERANK --> THRESHOLD{"Confidence >= 0.40?"}
        THRESHOLD -- Yes --> LLM["LLM Grounded Response"]
        THRESHOLD -- No --> FALLBACK["Polite Refusal (No Hallucination)"]
    end
```
