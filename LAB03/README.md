# ⚡ LAB03: LLM Retrieval System (RAG Lab)

ระบบค้นคืนข้อมูลและตอบคำถาม **RAG (Retrieval-Augmented Generation)** ที่ดึงคำตอบจากฐานความรู้ Q&A ผ่าน Embedding (`paraphrase-multilingual-MiniLM-L12-v2`) + FAISS Vector Database และแสดงผลผ่าน Web UI ด้วย Flask

---

### 👨‍💻 สมาชิกผู้จัดทำ (Author)
* **ชื่อ-นามสกุล:** นายรัชชานนท์ ศรีไชย (Ratchanon Srichai)
...
การติดตั้งครั้งแรกจะโหลดโมเดล embedding (`paraphrase-multilingual-MiniLM-L12-v2`) มาเก็บไว้ในเครื่องด้วย ต้องต่อเน็ตตอนติดตั้ง ใช้เวลาสักพัก
...
```
คำสั่งนี้ทำหน้าที่แทน Lab 1–4 ทั้งหมด (extract → chunk → embed → build index) ในคำสั่งเดียว จะได้ไฟล์ output ครบ:
