...
SOURCE_FILE = os.path.join(DATA_DIR, "ai_models_qa.txt")
...
# ============================================================
# Multi-dataset config (used by app.py / the web demo only).
# labs/*.py and main.py keep using the single SOURCE_FILE above
# so the original assignment steps still work unchanged.
# ============================================================
DATASETS = {
    "ai_models": {
        "label": "โมเดล AI ในปัจจุบัน",
        "source_file": os.path.join(DATA_DIR, "ai_models_qa.txt"),
        "chunks_file": os.path.join(OUTPUT_DIR, "ai_models_chunks.json"),
        "embeddings_file": os.path.join(OUTPUT_DIR, "ai_models_embeddings.npy"),
        "index_file": os.path.join(VECTOR_DB_DIR, "ai_models.index"),
        "chunk_store_file": os.path.join(VECTOR_DB_DIR, "ai_models_chunk_store.json"),
        "sample_questions": [
            "Claude มีโมเดลระดับไหนบ้าง",
            "งบประมาณจำกัดควรเลือกโมเดลแบบไหน",
            "RAG คืออะไร",
        ],
    },
}
